...
class Settings(BaseSettings):
    ...
    hf_device:int=1
    hf_max_new_token:int=Field(default=2048,ge=1)
    ...
    geration_retrieval_k:int=Field(default=16,ge=1,le=128)
    ...
    def validate_config(self)->"Settings":
        if self.chunk_overlap>=self.chunk_size:
            raise ValueError("chunk overlap must smallest chunk_size")
        if self.hf_device < -1:
